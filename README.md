...
They stay opt-in because they need `torch` and a model download, which the default must not. **But if you have installed the extra, turn them on.** Measured against a second fixture whose machine text was *not* written to blend into its surroundings, they find 12 of 75 machine-written sentences at 86% precision where the deterministic set finds 0 of 75 — and they falsely accuse human writers *less* often than the deterministic default does (24% vs 64% on the native control, before Phase 2's scorer change). The numbers, including the unflattering ones, are in [`phase-1-results.md`](misc/docs/phase-1-results.md) and [`phase-2-results.md`](misc/docs/phase-2-results.md); why a continuous score becomes a banded signal rather than a weight is [`curvature-as-evidence.md`](misc/docs/curvature-as-evidence.md).
